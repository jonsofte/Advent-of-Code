...
static int GetBasinSize((int r, int c) point, int[][] depthMap, HashSet<(int, int)> visitedPoints)
{
    ...
    foreach (var d in new (int r, int c)[] { (-1, 0), (1, 0), (0, -1), (0, 1) })
    {
        if (ValidPosition(point.r + d.r, point.c + d.c, depthMap) &&
            !visitedPoints.Contains((point.r + d.r, point.c + d.c)) &&
            depthMap[point.r + d.r][point.c + d.c] < 9)
        {
            sum += GetBasinSize((point.r + d.r, point.c + d.c), depthMap, visitedPoints);
        }
    }
    return 1 + sum;
}